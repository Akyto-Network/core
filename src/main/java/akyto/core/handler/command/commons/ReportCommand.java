...
public class ReportCommand {
	...
	public boolean reportCommand(final CommandArgs arg) {
		...
		if (args.length > 1) {
			...
			final TextComponent comp = new TextComponent(ChatColor.GRAY + "(" + ChatColor.RED + "!" + ChatColor.GRAY + ") " + ChatColor.GREEN + sender.getName() + ChatColor.GRAY + " have reported " + ChatColor.RED + (args[0] != target ? target + ChatColor.GRAY + "[" + ChatColor.YELLOW + ChatColor.ITALIC + args[0] + ChatColor.GRAY + "]" : target)+ ChatColor.GRAY + " for " + ChatColor.WHITE + reason);			comp.setHoverEvent(new HoverEvent(Action.SHOW_TEXT, new ComponentBuilder(Bukkit.getPlayer(target) != null ? ChatColor.DARK_GRAY + "Click to teleport to reported player!" : ChatColor.RED + "The reported is offline now!").create()));
